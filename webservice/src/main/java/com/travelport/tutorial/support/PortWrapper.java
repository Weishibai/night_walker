...
//        }
    ...
    protected void addParametersToProvider(BindingProvider provider,
            String endpoint,String user,String pwd) {
     ...
     */
    public void showXML(boolean show,String user,String password) throws FileNotFoundException {
        if (port==null) {
            init(user,password);
        }
        ...
        if (show) {
            if (!currentlyShown) {
            	in = new LoggingInInterceptor();
            	out = new LoggingOutInterceptor();
    ...
    protected void init(String user,String pwd) {
        try {
            ...
            port = (P)result;
            addParametersToProvider((BindingProvider) port, endpoint,user,pwd);
